...
import cn.edu.uestc.smgt.shiro.entity.OnlineSession.OnlineStatus;
...
public class KickoutSessionControlFilter extends AccessControlFilter {
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws Exception {
		...
		if (session != null && session instanceof OnlineSession) {
			...
			//
			//            if (onlineSession.getStatus() == OnlineSession.OnlineStatus.KICKOUT) {
			//                return false;
			//            }
		}
		return false;
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
		...
		synchronized (username) {
			...
			while(deque.size() > maxSession) {
				...
				try {
					OnlineSession kickoutSession = (OnlineSession)sessionManager.getSession(new DefaultSessionKey(kickoutSessionId));
