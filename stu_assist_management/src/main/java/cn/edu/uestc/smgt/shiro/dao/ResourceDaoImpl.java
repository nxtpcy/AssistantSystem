...
public class ResourceDaoImpl implements ResourceDao {
    ...
    public Resource createResource(final Resource resource) {
        final String sql = "insert into help_resource(name, type, url, permission, parent_id, parent_ids, available) values(?,?,?,?,?,?,?)";
    ...
    public Resource updateResource(Resource resource) {
        final String sql = "update help_resource set name=?, type=?, url=?, permission=?, parent_id=?, parent_ids=?, available=? where id=?";
        jdbcTemplate.update(
                sql,
    ...
    public void deleteResource(Integer resourceId) {
        ...
        final String deleteSelfSql = "delete from help_resource where id=?";
        jdbcTemplate.update(deleteSelfSql, resourceId);
        final String deleteDescendantsSql = "delete from help_resource where parent_ids like ?";
        jdbcTemplate.update(deleteDescendantsSql, resource.makeSelfAsParentIds() + "%");
    }


    public Resource findOne(Integer resourceId) {
        final String sql = "select id, name, type, url, permission, parent_id, parent_ids, available from help_resource where id=?";
    ...
    public List<Resource> findAll() {
        final String sql = "select id, name, type, url, permission, parent_id, parent_ids, available from help_resource order by concat(parent_ids, id) asc";
